...
class CourseSnail extends BaseSnail
{
    protected function course($url, $query, $p, $term)
    {
        empty($p) ? $p = 1 : $p = 0;
        $this->name = $this->name($this->viewstate);
        $curlArg = array(
            'url' => $url . $this->studentcode . "&xm=" . $this->name, //. '&gnmkdm=N121601',
            'method' => 'post',
            'data' => $term,
            'responseHeaders' => 0,
        ...
        foreach ($table as $x => $z) {
            if (preg_match('/第[13579]节/', $z[0])) {
                for ($i = 0; $i <= 7; $i++) {
                    ...
                    if (!isset($table[$x + $i + 1][2])) unset($schedule[$m + $i]);  //课程开设时间不存在，删除该课程
                    else {
    ...
    public function class_course($studentcode, $password, $term)
    {
        ...
        $p = '';
        return $this->course($url, $query, $p, $term);
    }

    public function personal_course($studentcode, $password, $term)
    {
        $this->studentcode = $studentcode;
        $this->password = $password;
